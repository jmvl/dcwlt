...
const isDOMAvailable = _reactNative.Platform.OS === 'web';
...
var _default = {
  ...
  async openAuthSessionAsync(url, redirectUrl, openOptions) {
    ...
    return new Promise(async resolve => {
      ...
      const appStateSubscription = _reactNative.AppState.addEventListener('change', appStateListener); // Check if the window has been closed every second.
...
function getPopupFeaturesString(options) {
  ...
  const dimensions = _reactNative.Dimensions.get('screen');

  const top = windowFeatures.top ?? Math.max(0, (dimensions.height - height) * 0.5);
  const left = windowFeatures.left ?? Math.max(0, (dimensions.width - width) * 0.5); // Create a reasonable popup

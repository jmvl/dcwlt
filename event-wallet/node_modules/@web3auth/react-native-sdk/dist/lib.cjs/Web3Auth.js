...
var KeyStore = require('./session/KeyStore.js');
...
class Web3Auth {
  constructor(webBrowser, storage, options) {
    ...
    if (!options.privateKeyProvider) {
      throw errors.InitializationError.invalidParams("Please provide privateKeyProvider");
    }
    if (!options.network) options.network = auth.WEB3AUTH_NETWORK.SAPPHIRE_MAINNET;
    if (!options.buildEnv) options.buildEnv = auth.BUILD_ENV.PRODUCTION;
    if (options.buildEnv === auth.BUILD_ENV.DEVELOPMENT || options.buildEnv === auth.BUILD_ENV.TESTING || options.sdkUrl) this.addVersionInUrls = false;
    if (!options.sdkUrl && !options.useMpc) {
      if (options.buildEnv === auth.BUILD_ENV.DEVELOPMENT) {
        options.sdkUrl = "http://localhost:3000";
        options.dashboardUrl = "http://localhost:5173/wallet/account";
      } else if (options.buildEnv === auth.BUILD_ENV.STAGING) {
        options.sdkUrl = "https://staging-auth.web3auth.io";
        options.dashboardUrl = "https://staging-account.web3auth.io/wallet/account";
      } else if (options.buildEnv === auth.BUILD_ENV.TESTING) {
    ...
    if (options.useMpc && !options.sdkUrl) {
      if (Object.values(auth.WEB3AUTH_NETWORK).includes(options.network)) throw errors.InitializationError.invalidParams("MPC is not supported on legacy networks, please use sapphire_devnet or sapphire_mainnet.");
      if (options.buildEnv === auth.BUILD_ENV.DEVELOPMENT) {
        options.sdkUrl = "http://localhost:3000";
      } else if (options.buildEnv === auth.BUILD_ENV.STAGING) {
  ...
  async init() {
    if (!this.privateKeyProvider.currentChainConfig || !this.privateKeyProvider.currentChainConfig.chainNamespace || !this.privateKeyProvider.currentChainConfig.chainId) {
      throw errors.InitializationError.invalidParams("provider should have chainConfig and should be initialized with chainId and chainNamespace");
    }
    ...
    let projectConfig;
    try {
      projectConfig = await utils.fetchProjectConfig(this.options.clientId, this.options.network);
    } catch (e) {
  ...
  async login(loginParams) {
    ...
    if (!this.ready) throw errors.InitializationError.notInitialized("Please call init first.");
    if (!loginParams.redirectUrl && !this.options.redirectUrl) throw errors.InitializationError.invalidParams("redirectUrl is required");
    if (!loginParams.loginProvider) throw errors.InitializationError.invalidParams("loginProvider is required");
    ...
    const dataObject = {
      actionType: auth.AUTH_ACTIONS.LOGIN,
    ...
    if (result.type !== "success" || !result.url) {
      ...
      throw errors.LoginError.loginFailed(`login flow failed with error type ${result.type}`);
    }
    const {
      sessionId,
      sessionNamespace,
      error
    } = utils.getHashQueryParams(result.url);
    if (error || !sessionId) {
      throw errors.LoginError.loginFailed(error || "SessionId is missing");
    }
    if (sessionId) {
    ...
    if (!sessionData || Object.keys(sessionData).length === 0) {
      throw errors.LoginError.loginFailed("Session data is missing");
    }
    ...
    const finalPrivKey = this.getFinalPrivKey();
    if (!finalPrivKey) throw errors.LoginError.loginFailed("final private key not found");
    await this.privateKeyProvider.setupProvider(finalPrivKey);
    // setup aa provider after private key provider is setup
    if (this.accountAbstractionProvider) {
      await this.accountAbstractionProvider.setupProvider(this.privateKeyProvider);
  ...
  async launchWalletServices(chainConfig, path = "wallet") {
    ...
    const loginUrl = utils.constructURL({
      baseURL: url,
      hash: {
        b64Params: auth.jsonToBase64(configParams)
      }
    });
  ...
  async request(chainConfig, method, params, path = "wallet/request") {
    ...
    if (error || success === "false" || !success) {
      throw errors.RequestError.fromCode(5000, error);
    }
  ...
  async enableMFA() {
    ...
    if (this.state.userInfo.isMfaEnabled) {
      throw errors.LoginError.mfaAlreadyEnabled();
    }
    const loginParams = {
      ...
      mfaLevel: auth.MFA_LEVELS.MANDATORY,
      extraLoginOptions: {
    ...
    const dataObject = {
      actionType: auth.AUTH_ACTIONS.ENABLE_MFA,
      options: this.options,
      params: _objectSpread(_objectSpread({}, loginParams), {}, {
        redirectUrl: loginParams.redirectUrl || this.options.redirectUrl,
        mfaLevel: "mandatory"
    ...
    if (result.type !== "success" || !result.url) {
      ...
      throw errors.LoginError.loginFailed(`enableMFA flow failed with error type ${result.type}`);
    }
    const {
      sessionId,
      sessionNamespace,
  ...
  getFinalPrivKey() {
    if (this.options.privateKeyProvider.currentChainConfig.chainNamespace === constants.CHAIN_NAMESPACES.SOLANA) return this.getFinalEd25519PrivKey();
  ...
  getFinalCommonPrivKey() {
    ...
    if (this.options.useCoreKitKey) {
      ...
      if (this.state.privKey && !this.state.coreKitKey) {
        throw errors.LoginError.coreKitKeyNotFound();
      }
